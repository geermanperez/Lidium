...
// <editor-fold defaultstate="collapsed" desc="Packet Log Implementation">
        ...
        for (final RecvPacketOpcode recv : RecvPacketOpcode.values()) {
            if (recv.getValue() == header_num) {
                ...
                try {
                    ...
                    logCharacterSelectionException(recv, slea, e);
                    if (!ServerConstants.Use_Fixed_IV) {
                        FileoutputUtil.outputFileError(FileoutputUtil.PacketEx_Log, e);
    ...
    private static void logCharacterSelectionException(final RecvPacketOpcode recv, final LittleEndianAccessor slea, final Exception e) {
        if (recv == RecvPacketOpcode.PICK_ALL_CHAR || recv == RecvPacketOpcode.CHAR_SELECT_NO_PIC
                || recv == RecvPacketOpcode.VIEW_REGISTER_PIC || recv == RecvPacketOpcode.CHAR_SELECT
                || recv == RecvPacketOpcode.VIEW_SELECT_PIC || recv == RecvPacketOpcode.AUTH_SECOND_PASSWORD) {
            System.err.println("[CHAR LOGIN DEBUG] handler=" + recv + " action=packet-exception"
                    + " bytesRemaining=" + slea.available() + " exception=" + e);
        }
    }
